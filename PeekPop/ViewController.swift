...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        tableView.transform = CGAffineTransformMakeRotation(.pi)
    ...
    @objc func handleLongPress(_ gesture: UILongPressGestureRecognizer) {
        if gesture.state == .began {
            ...
            // Use longPressLocation to get the position of the long press
            // ...
            contextMenuLocation = longPressLocation
        }
    }
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func makeTargetedPreview(for configuration: UIContextMenuConfiguration) -> UITargetedPreview? {
        ...
        snapshot.transform = CGAffineTransformMakeRotation(.pi)
        ...
        reactionView.transform = CGAffineTransformMakeRotation(.pi)
        let cellBounds = cell.bounds
                let cellCenter = CGPoint(x: cellBounds.midX, y: cellBounds.midY)
        let convertedCenter = cell.convert(cellCenter, to: tableView)
               let convertedCenterInWindow = tableView.convert(convertedCenter, to: nil)
        let centerPoint = CGPoint(x: convertedCenterInWindow.x, y:  convertedCenterInWindow.y )
        let previewTarget = UIPreviewTarget(container: self.view, center: centerPoint)
        ...
        preview.view.transform = CGAffineTransformMakeRotation(.pi)
        return preview
    }
    ...
    func makeTargetedDismissPreview(for configuration: UIContextMenuConfiguration) -> UITargetedPreview? {
        ...
        let centerPoint = CGPoint(x: cell.center.x - self.view.center.x, y: self.view.center.y)
        let previewTarget = UIPreviewTarget(container: self.view, center: centerPoint)
        let parameters = UIPreviewParameters()
        parameters.backgroundColor = .clear
        if #available(iOS 14.0, *) {
